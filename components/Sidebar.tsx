...
const Sidebar: React.FC<SidebarProps> = ({ currentPage, setCurrentPage, isOpen, setIsOpen, onLogout }) => {
  ...
  const handleLogoutClick = () => {
    if (confirm("Apakah Anda yakin ingin keluar dari Dashboard Guru?")) {
        onLogout();
    }
  };
      ...
      >
        <div className="h-full flex flex-col">
          ...
          <div className="flex items-center gap-3 px-6 h-16 border-b border-gray-200">
            ...
            <span className="text-xl font-bold text-slate-800">GuruPintar</span>
          </div>
          ...
          <div className="p-4 border-t border-gray-200 bg-gray-50">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-gray-900 truncate">Guru Admin</p>
                <p className="text-xs text-gray-500 truncate">Online</p>
              </div>
            </div>
            <button 
                onClick={handleLogoutClick}
                className="w-full flex items-center justify-center gap-2 bg-white text-red-600 border border-red-200 px-4 py-2 rounded-lg text-sm font-medium hover:bg-red-50 transition-colors"
            >
